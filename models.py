#the concat version with the att mechanism attached.
...
class OverAll(nn.Module):
    def __init__(self, node_size, node_hidden, time_hidden,
        ...
        self.r_encoder = GraphAttention(node_size, rel_size, triple_size, time_size, depth=depth, device=device,
                                        dim=node_hidden)
        self.t_encoder = GraphAttention_(node_size, rel_size, triple_size, time_size, depth=depth, device=device,
                                         dim=time_hidden)
        ...
        self.time_adj = self.get_spares_matrix_by_index_value\
        (time_matrix, time_val, (node_size, time_size))
        
        self.time_int_adj = self.get_spares_matrix_by_index_value\
    ...
    def forward(self, inputs):
        ...
        #time_feature = (time_feature + time_int_feature) / 2
        ...
        opt = [self.rel_emb, adj_input]
        opt2 = [self.time_emb, adj_input, self.time_emb[0]]
        opt3 = [self.time_emb, adj_input, self.time_int_emb[1101]]
        ...
        out_feature_time = self.t_encoder([time_feature] + opt2, 1)
        out_feature_int_time = self.t_encoder([time_int_feature] + opt3, 1)
        ...
        if global_args.dual_no_time:
            ...
            out_feature_overall = torch.cat((out_feature_rel, out_feature_time), dim=-1)
...
class GraphAttention_(nn.Module):
    ...
    def forward(self, inputs, rel_or_time=0):
        ...
        none_temporal = inputs[3] if rel_or_time == 1 else None
        ...
        #    rels_sum = torch.matmul(rels_sum, rel_emb)
            ...
            sim_score = torch.squeeze(-torch.matmul(F.normalize(neighs, dim=-1), \
                                     torch.transpose(F.normalize(none_temporal.reshape(1, -1), dim=-1), 0, 1)), dim=-1)
            #print(sim_score.shape)
            sim_att = torch.sparse.FloatTensor(torch.transpose(index, 0, 1), sim_score, (self.node_size, self.node_size))
            sim_att = torch.sparse.softmax(sim_att, dim=1)
        ...
        #    att = torch.sparse.FloatTensor(torch.transpose(index, 0, 1), att1, (self.node_size, self.node_size))
            # ??? dim ??
            #the same weight version.
            att_ = torch.sparse.FloatTensor(torch.transpose(index, 0, 1), torch.ones_like(index[:, 0], dtype=torch.float), \
                                            (self.node_size, self.node_size))

            att = torch.sparse.softmax(att_, dim=1)
            # ?
            # print(att1)
            # print(att.data)
