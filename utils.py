...
def get_matrix(triples, entity, rel, time, time_int, ent_tem_time_count,\
    ...
    for i in range(len(ent_tem_time_count)):
        ...
        for j in tem_times_rec:
            ...
            tf = np.log(1 + time_j)
            #time_link[i][j] = tf
            time_link[i][j] = tf if j in tem_valid_set else -100
    ...
    for i in range(len(ent_tem_int_time_count)):
        ...
        for j in tem_times_rec:
            ...
            time_int_link[i][time_int_dict_i[j]] = tf if j in tem_int_valid_set else -100
    ...
    time_features, time_int_features, radj, time_int_dict_i


def load_data(lang, train_ratio=0.3, ent_int_dict=None, valid_int_set=None, unsup=False):
    ...
    if ent_int_dict is None:
        ent_int_dict = ent_tem_int_time_count
    tem_valid_set = time1 & time2
    tem_int_valid_set = time_int1 & time_int2
    if valid_int_set is None:
        valid_int_set = tem_int_valid_set
    ...
    adj_matrix, adj_features, rel_features, time_features, \
    time_int_features, radj, time_int_dict_i= \
        get_matrix(triples1 + triples2, \
    entity1.union(entity2), rel1.union(rel2), time1.union(time2), time_int1.union(time_int2),\
         ent_tem_time_count, ent_int_dict, tem_valid_set, valid_int_set)

    return np.array(train_pair), np.array(dev_pair), \
    adj_matrix, adj_features, rel_features, time_features, time_int_features, \
        radj, time_int_dict_i, ent_int_dict